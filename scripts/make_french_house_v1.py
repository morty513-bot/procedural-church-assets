...
OUT_DIR = os.path.join(REPO_ROOT, 'assets', 'french_house_v1')
os.makedirs(OUT_DIR, exist_ok=True)

BLEND_PATH = os.path.join(OUT_DIR, 'french_house_v1.blend')
GLB_PATH = os.path.join(OUT_DIR, 'french_house_v1.glb')
...
def assert_on_ground(primary_base_obj, objects, eps=1e-4):
    # Main shell must sit on ground and no part should sink under ground plane.
...
def render_views(scene, cam, look_at, radius, out_dir, prefix='preview_wide', angles=(35, 115, 210), z=3.2):
    paths = []
    for i, deg in enumerate(angles, start=1):
        rad = math.radians(deg)
        cam.location = Vector((look_at.x + radius * math.cos(rad), look_at.y + radius * math.sin(rad), z))
...
def render_detail(scene, cam, focus, out_path):
    cam.location = Vector((focus.x + 2.8, focus.y - 2.4, focus.z + 1.0))
...
# Materials
m_stone = mat('StoneWallV1', base=(0.69, 0.66, 0.60, 1.0), rough=0.93)
m_plaster = mat('PlasterV1', base=(0.82, 0.78, 0.70, 1.0), rough=0.88)
m_timber = mat('TimberV1', base=(0.33, 0.22, 0.15, 1.0), rough=0.86)
m_thatch = mat('ThatchV1', base=(0.60, 0.49, 0.25, 1.0), rough=0.95)
m_door = mat('DoorWoodV1', base=(0.24, 0.15, 0.10, 1.0), rough=0.7)
m_window = mat('WindowFrameV1', base=(0.18, 0.21, 0.23, 1.0), rough=0.52)
m_ground = mat('GroundV1', base=(0.35, 0.46, 0.30, 1.0), rough=0.97)
...
# Slightly overhanging low-poly thatched roof
bpy.ops.mesh.primitive_cone_add(vertices=4, radius1=3.25, radius2=0.22, depth=2.8, location=(0, 0, 4.75), rotation=(0, 0, math.radians(45)))
roof = bpy.context.active_object
roof.data.materials.append(m_thatch)
meshes.append(roof)
building_parts.append(roof)

# Roof ridge cap (simple cylinder to sharpen silhouette)
bpy.ops.mesh.primitive_cylinder_add(vertices=8, radius=0.12, depth=4.8, location=(0, 0, 5.48), rotation=(math.radians(90), 0, 0))
...
# Side dormer-ish tiny windows (adds French village character)
for y in (-0.65, 0.65):
...
# Parent for clean scene organization
bpy.ops.object.empty_add(type='PLAIN_AXES', location=(0, 0, 0))
root = bpy.context.active_object
root.name = 'FrenchHouseV1'
...
# Camera + renders
bpy.ops.object.camera_add(location=(0, -11, 3.2))
cam = bpy.context.active_object
scene.camera = cam

_, _, center, size = compute_bounds(building_parts)
look = center.copy()
look.z = center.z + 0.4
radius = max(size.x, size.y) * 2.5

render_views(scene, cam, look, radius, OUT_DIR, prefix='preview_wide', angles=(35, 115, 215), z=3.3)
render_detail(scene, cam, Vector((0, -1.1, 1.65)), os.path.join(OUT_DIR, 'preview_detail.png'))
